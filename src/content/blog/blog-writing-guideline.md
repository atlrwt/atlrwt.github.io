---
title: "本站文档写作规范"
description: "沉淀本站写作规范，覆盖文章结构、语言风格、排版细节和技术文档表达要求。"
publishDate: "2025-03-28T21:22:35+08:00"
categories:
  - "Blog"
tags:
  - "Writing"
...
本文转载自阮一峰的[中文技术文档的写作规范](https://github.com/ruanyf/document-style-guide?tab=readme-ov-file)。

在此基础上针对个人风格进行了适当调整，并添加了涉及到代码变量部分的写作建议（5.7 反引号）。

全文示例采用 markdown 格式。

本站之后的文章将统一在该规范的指导下进行编写。
...
示例：下面的文章结构，二级标题 A 只包含一个三级标题，完全可以省略三级标题 A。
...
（4）谨慎使用四级标题，尽可能不用，保持层级的简单，防止出现过于复杂的章节。
...
```markdown
欠佳：本文介绍如何快速启动Windows系统。

推荐：本文介绍如何快速启动 Windows 系统。
```

（2）全角中文字符与半角阿拉伯数字之间，应有一个半角空格。

（注：原文这里的建议是有没有半角空格都可，但必须保证风格统一，不能两种风格混杂。）

```markdown
欠佳：2011年5月15日，我订购了5台笔记本电脑与10台平板电脑。
...
推荐：1 h = 60 min = 3,600 s
```

（4）半角英文字符和半角阿拉伯数字，与全角标点符号之间不留空格。

```markdown
欠佳：他的电脑是 MacBook Air 。
...
```markdown
欠佳：本产品适用于从由一台服务器进行动作控制的单一节点结构到由多台服务器进行动作控制的并行处理程序结构等多种体系结构。

推荐：本产品适用于多种体系结构。无论是由一台服务器（单一节点结构），还是由多台服务器（并行处理结构）进行动作控制，均可以使用本产品。
```
...
```markdown
欠佳（复合句）：那个昨天生病的人没有参加会议。

推荐（并列句）：他昨天生病了，没有参加会议。
```
...
表示数值范围时，用波浪线（～）或一字线（—）连接。参见“五、标点符号”一节的“[连接号](#conjunction)”部分。
...
```markdown
欠佳：我最欣赏的科技公司有 Google、Facebook、腾讯、阿里，以及百度等。
...
在技术博客中，处理变量名、函数名、代码片段等编程相关元素时，使用行内代码块（即用反引号 `` 包裹）来突出显示。

示例：

```markdown
欠佳：在 SQL 中，变量 tbl 通常表示数据表的引用。

推荐：在 SQL 中，变量 `tbl` 通常表示数据表的引用。
```

反引号的适用场景如下：

- 变量名、函数名：如 `count`，`calculate_score()`。
- 代码片段：短代码（如 `if x > 0:`）或跨行的代码块（用三个反引号包裹）。
- 文件路径/命令：如 `/path/to/file`, `npm install`。

应注意避免滥用：仅对需要强调的编程术语使用反引号，普通词汇无需包裹（例如“函数”这个词本身不需要加反引号）。
...
- [Atom 手册](https://github.blog/news-insights/product-news/sunsetting-atom/)
